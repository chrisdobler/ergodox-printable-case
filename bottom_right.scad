...
module build(){
  ...
  // rightSection();
  // cover();
  // mirror() translate([90,102,-1]) puzzlePiece();
  // mirror() translate([150,102,-1]) puzzlePiece();
}
build();

module cover() {
  mirror() {
    translate([0,0,-1]) // 20,46
      cube([230,280,9]);  
  } 
}
...
module thumbPart() {
  difference() {
    ...
    difference() {
      cover();
      difference() {
        mirror() translate([60,0,-1]) // 20,46
          color("green")
            cube([175,110,8]);
        union() {
          rightSection();
          mirror() translate([73.5,77,-1]) rotate([0,0,90]) puzzlePiece();
          mirror() translate([90,102,-1]) puzzlePiece();
          mirror() translate([150,102,-1]) puzzlePiece();
        }
      }
    ...
    difference() {
      ...
      color("purple") {
        difference() {
          union() {
            mirror() {
              translate([15,160,-1]) // 20,46
                cube([115,70,9]);
            }
            //bottom part
            mirror() {
              translate([-20,40,-1]) // 20,46
                cube([85,140,9]);  
            } 
          }
          // thumbPart();
